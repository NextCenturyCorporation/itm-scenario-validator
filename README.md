...
| `deepLinks` | "If [field1] has a value of [value1] and [field2] has a value of [value2], then [field3] values must be [...]" | An object where each key is a shared parent of all fields throughout the rest of the object. For example, state.characters[].vitals is the parent of conscious, avpu, and mental_status. Each value contains "sharedParent", "condition" and "requirement".  "condition" is the set of fields and values that must be found in order for "requirement" to be required. "requirement" is a set of fields matched to their allowed values given the conditions |
